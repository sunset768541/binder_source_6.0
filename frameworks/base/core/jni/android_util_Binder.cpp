...
//Binder.java的init()方法native的实现类
static void android_os_Binder_init(JNIEnv* env, jobject obj)
{
    JavaBBinderHolder* jbh = new JavaBBinderHolder();//创建一个JavaBBinderHolder的实例
    if (jbh == NULL) {
        jniThrowException(env, "java/lang/OutOfMemoryError", NULL);
        return;
    }
    ALOGV("Java Binder %p: acquiring first ref on holder %p", obj, jbh);
    jbh->incStrong((void*)android_os_Binder_init);//强指针加1 (void*)android_os_Binder_init方法的地址
...
static jobject android_os_BinderInternal_getContextObject(JNIEnv* env, jobject clazz)//返回给java层一个jobjct对象
{
    sp<IBinder> b = ProcessState::self()->getContextObject(NULL);//BpBinder(0)  
    return javaObjectForIBinder(env, b);//将BpBinder（0）转换为jobjec返回给java层
}
