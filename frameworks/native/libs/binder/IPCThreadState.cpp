                                  ...
                                  Parcel* reply, uint32_t flags)
{
    ...
    if ((flags & TF_ONE_WAY) == 0) {
        ...
        #endif
        if (reply) {//从ipcthread发起跨进程传输，调用binder驱动，等待数据返回
...
status_t IPCThreadState::talkWithDriver(bool doReceive)
{
    ...
    do {
        ...
        if (ioctl(mProcess->mDriverFD, BINDER_WRITE_READ, &bwr) >= 0)//写入binder
            err = NO_ERROR;
        else
            err = -errno;
#else
