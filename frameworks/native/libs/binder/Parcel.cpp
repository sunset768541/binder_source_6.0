...
{
    flat_binder_object obj;//此时在用户空间

    obj.flags = 0x7f | FLAT_BINDER_FLAG_ACCEPTS_FDS;
    if (binder != NULL) {
        IBinder *local = binder->localBinder();//BBinder local不为空
        if (!local) {//对于local为真不走这里
            BpBinder *proxy = binder->remoteBinder();//对于Bpinder remoteBinder不为空
            if (proxy == NULL) {
                ALOGE("null proxy");
            }
            const int32_t handle = proxy ? proxy->handle() : 0;//proxy空 handler = 0
            ...
            obj.binder = reinterpret_cast<uintptr_t>(local->getWeakRefs());//获取若引用指针
            obj.cookie = reinterpret_cast<uintptr_t>(local);//获取Binder实体的用户空间指针
        }
    } else {
    ...
    return finish_flatten_binder(binder, obj, out);//将obj数据写入out
}
