                   ...
                   pid_t spid)
{
    ...
    if (si) {
        ...
        si->next = svclist;//更新服务的链表
        svclist = si;//服务链表的头指向最新加入的Service
    }
                   ...
                   struct binder_io *reply)
{
    ...
    s = bio_get_string16(msg, &len);//通过msg给len赋值
    if (s == NULL) {
        return -1;
    }
    ...
    case SVC_MGR_CHECK_SERVICE:
        ...
        if (!handle)
            break;
        bio_put_ref(reply, handle);//根据服务名称找到了服务的handle，并将handle放到reply中
        return 0;

    case SVC_MGR_ADD_SERVICE:
        s = bio_get_string16(msg, &len);//获取服务的名字
        if (s == NULL) {
            return -1;
        }
        handle = bio_get_ref(msg);
        allow_isolated = bio_get_uint32(msg) ? 1 : 0;
        //s是什么 len是什么，ams的handle是多少?
...
int main(int argc, char **argv)
{
    ...
    bs = binder_open(128*1024);//在service manager的main方法中打开binder_驱动 只有这里调用了binder——open分配了128kb的大小
    if (!bs) {
        ALOGE("failed to open binder driver\n");
        return -1;
    }

    if (binder_become_context_manager(bs)) {//BINDER_SET_CONTEXT_MGR命令将自己注册成SMgr时Binder驱动会自动为它创建Binder实体
